        ...
        out.append(
            {
                ...
                "metadata": {
                    "n_atoms": len(atoms),
                    "atom_counts": dict(collections.Counter(atoms)),
                },
            }
        )
    return out
...
def main() -> None:
    ...
    leaked = [
        k for s in shareables for k in s if k not in {"trace_id", "agent", "atoms", "metadata"}
    ]
    ...
    print(
        f"exported {len(shareables)} session(s): {n_cc} Claude Code, {n_cursor} Cursor, {total_atoms} atoms"
    )
    print(f"  {out}")
