...
PAL = {"paper": "#F7F5F2", "ink": "#14110E", "copper": "#CB4D20",
       "blue": "#5692E5", "olive": "#585E53", "rule": "#d9d4cc"}
...
def claude_code_runs(path: str = "~/.claude/projects", recent: int = 30) -> list[int]:
    ...
    return [len(t["seq"]) for f in files[-recent:] for t in build_panel_session({"events": _lines(f)})["turns"]]


def cursor_runs(db: str, limit: int = 60) -> list[int]:
    return [len(t["seq"]) for s in build_panel_sessions(Path(db).expanduser(), limit=limit) for t in s["turns"]]
...
def _panel(x0: float, title: str, runs: list[int], ymax: float, color: str) -> str:
    ...
    for i, f in enumerate(fr):
        if not f:
            continue
        h = (f / ymax) * ph
        bars += (f'<rect x="{x0 + i * bw + 0.6:.1f}" y="{mt + ph - h:.1f}" '
                 f'width="{bw - 1.2:.1f}" height="{h:.1f}" fill="{color}"/>')
    xt = ""
    for v in (1, 8, 64, 512):
        i = _bin_index(v)
        x = x0 + (i + 0.5) * bw
        xt += (f'<text x="{x:.1f}" y="{mt + ph + 15:.0f}" text-anchor="middle" '
               f'font-size="10" fill="{PAL["olive"]}">{v}</text>')
    base = (f'<line x1="{x0:.1f}" y1="{mt + ph:.1f}" x2="{x0 + pw:.1f}" y2="{mt + ph:.1f}" '
            f'stroke="{PAL["rule"]}" stroke-width="1"/>')
    head = (f'<text x="{x0:.1f}" y="18" font-size="12" font-weight="600" fill="{PAL["ink"]}">{title}</text>'
...
def build_html(runs: dict[str, list[int]]) -> str:
    ...
    yl = (f'<text x="14" y="34" font-size="10" fill="{PAL["olive"]}">{ymax * 100:.0f}%</text>'
          f'<text x="14" y="198" font-size="10" fill="{PAL["olive"]}">0</text>')
    svg = (f'<svg viewBox="0 0 {sw} {sh}" width="100%" role="img" '
    ...
    return (f'<title>procgrep · run length by surface</title>'
...
def main() -> None:
    ...
    parser.add_argument("--cursor", default="~/Library/Application Support/Cursor/User/globalStorage/state.vscdb")
